...
from PyQt5.QtWidgets import QWidget, QVBoxLayout, QSizePolicy, QInputDialog
...
from col_var import critical, test_latex
...
class MatplotlibWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.__cid = self.canvas.mpl_connect("scroll_event", self.do_scrollZoom)  # 支持鼠标滚轮缩放
        self.__cid1 = self.canvas.mpl_connect("pick_event", self.do_series_pick)  # 支持曲线抓取
        # self.__cid2 = self.canvas.mpl_connect("button_press_event",self.do_pressMouse)#支持鼠标按下
        self.__cid3 = self.canvas.mpl_connect("button_release_event", self.do_releaseMouse)  # 支持鼠标释放
        self.__cid4 = self.canvas.mpl_connect("motion_notify_event", self.do_moveMouse)  # 支持鼠标移动
    ...
    def do_series_pick(self, event):  # picker事件获取抓取曲线
        ...
        if isinstance(event.artist, mpl.axis.Axis):
            ...
            while True:
                String, ok = QInputDialog.getText(self, "更改标注", "请输入新的标注",
                                                  text=String)
                if ok is True:
                    try:
                        test_latex(String)
                    except Exception as e:
                        critical(self, e, tit='非法输入', txt='请检查输入内容是否符合latex语法')
                    else:
